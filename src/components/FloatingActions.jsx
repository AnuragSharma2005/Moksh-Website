    ...
    <AnimatePresence>
      {isVisible && visibleActions?.length > 0 && (
        ...
        >
          ...
          {/* Quick Contact Info */}
          {!dismissedActions?.contactInfo && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.5 }}
              className="bg-gradient-to-r from-primary to-secondary border border-border rounded-lg p-3 shadow-lg max-w-xs relative group"
            >
              <motion.button
                onClick={() => dismissAction('contactInfo')}
                className="absolute -top-2 -left-2 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
              >
                <Icon name="X" size={12} />
